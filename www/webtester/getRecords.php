...
if ($id) {
	...
	if ($xfilter_status)
		$wh.=" and parsed_data REGEXP '\"status\":\"[^\"]*".mysql_real_escape_string($xfilter_status)."'";
//		$wh.=" and status like '%".mysql_real_escape_string($xfilter_status)."%'";
	else if ($xfilter_status_nonempty)
		$wh.=" and not(parsed_data REGEXP '\"status\":\"\"')";
//		$wh.=" and status<>''";
	...
	if ($count) {
		if ($_POST['type'] == 'LIST') {
			...
			while ($r=mysql_fetch_assoc($q)) {
				$r= array_merge($r, json_decode($r['raw_data'],true), json_decode($r['parsed_data'],true));
				$re[]=$r;
...
//	print_r($re); echo "\n";
	...
	foreach ($highlight as $hk => $hv) {
		$m=array();
		$f='Parse'.$hv;
		$vp->$f(json_decode($re[$i]['raw_data'],true), json_decode($re[$i]['parsed_data'],true), $m);
		foreach ($m as $mi) {
			$m2=array();
			if (!$mi['matches'][0])
				continue;
...
*/
			...
			if (count($m2)) {
				...
				if (!$re[$i]['highlight_'.$hk])
					$re[$i]['highlight_'.$hk]=$hi_text;
				else	$re[$i]['highlight_'.$hk].="<hr>".$hi_text;
